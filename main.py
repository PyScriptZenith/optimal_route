...
from schemas import Route, Point
...
async def upload_file_and_create_route(file: UploadFile = File(...)):
    ...
    if len(points_for_API_request) > 50:
        return JSONResponse(
            status_code=400,
            content={"message": "Максимальное количество точек для маршрута - 50."},
        )
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"message": "Невозможно построить маршрут."}
        )
...
@app.get(
    "/api/routes/{route_id}", response_model=RouteCreate, status_code=status.HTTP_200_OK
)
